...
USER_DATA_DIR = os.path.join(DEFAULT_ROOT, 'ngram')


class NGram:
    def __init__(self, model_name_or_path=None, cache_folder=USER_DATA_DIR):
        if model_name_or_path and os.path.exists(model_name_or_path):
            ...
            language_model_path = http_get(url, cache_folder, download_file_name='zh_giga.no_cna_cmn.prune01244.klm')
        try:
            import kenlm
        except ImportError:
