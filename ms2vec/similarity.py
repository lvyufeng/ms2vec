...
def cos_sim(a: Tensor, b: Tensor):
    ...
    return ops.mm(a_norm, b_norm.swapaxes(0, 1))
...
def semantic_search(query_embeddings: Tensor,
    ...
    if isinstance(query_embeddings, (np.ndarray, np.generic)):
        query_embeddings = Tensor.from_numpy(query_embeddings)
    ...
    if isinstance(corpus_embeddings, (np.ndarray, np.generic)):
        corpus_embeddings = Tensor.from_numpy(corpus_embeddings)
    ...
    for query_start_idx in range(0, len(query_embeddings), query_chunk_size):
        ...
        for corpus_start_idx in range(0, len(corpus_embeddings), corpus_chunk_size):
            ...
            # Get top-k scores
            cos_scores_top_k_values, cos_scores_top_k_idx = ops.topk(cos_scores, min(top_k, len(cos_scores[0])),
