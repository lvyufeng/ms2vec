...
from mindnlp.utils import logging, http_get
DEFAULT_ROOT = os.path.join(os.getcwd(), ".ms2vec")

logger = logging.get_logger(__name__)

pwd_path = os.path.abspath(os.path.dirname(__file__))
default_stopwords_file = os.path.join(pwd_path, 'stopwords.txt')
USER_DATA_DIR = os.path.join(DEFAULT_ROOT, 'word2vec')
...
class Word2Vec:
    ...
    model_key_map = {
        'tencent-ailab-embedding-zh-d200-v0.1.0': {
            'tar_filename': 'tencent-ailab-embedding-zh-d200-v0.1.0.tar.gz',
            'url': 'https://ai.tencent.com/ailab/nlp/en/data/tencent-ailab-embedding-zh-d200-v0.1.0.tar.gz',
            'binary': False,
            'untar_filename': 'Tencent_AILab_ChineseEmbedding.txt'},
        'light-tencent-chinese': {
            'tar_filename': 'light-tencent-chinese.bin',
            'url': 'https://hf-mirror.com/lvyufeng/light-tencent-chinese/resolve/main/light-tencent-chinese.bin?download=true',
            'binary': True,
            'untar_filename': 'light-tencent-chinese.bin'},
    }

    def __init__(self, model_name_or_path: str = 'light-tencent-chinese',
        ...
        if model_name_or_path and os.path.exists(model_name_or_path):
            ...
            model_dict = self.model_key_map.get(model_name_or_path, self.model_key_map['light-tencent-chinese'])
            ...
            if not os.path.exists(model_name_or_path):
                ...
                download_path = http_get(url, cache_folder, download_file_name=tar_filename)
                if 'tar' in download_path:
                    pass
